package com.dpanayotov.gameoflife.preferences;

/**
 * Created by Dean Panayotov on 9/26/2016
 */

public class Keys {

    public static final String RESOLUTIONS = "RESOLUTIONS";
    public static final String RESOLUTION = "RESOLUTION";
    public static final String COLOR_PRIMARY = "COLOR_PRIMARY";
    public static final String COLOR_BACKGROUND = "COLOR_BACKGROUND";
    public static final String TICK_RATE = "TICK_RATE";
    public static final String ISOMETRIC_PROJECTION = "ISOMETRIC_PROJECTION";
    public static final String HIGHLIFE = "HIGHLIFE";
    public static final String MIN_POPULATION_DENSITY = "MIN_POPULATION_DENSITY";
    public static final String INITIAL_POPULATION_DENSITY = "INITIAL_POPULATION_DENSITY";

}
